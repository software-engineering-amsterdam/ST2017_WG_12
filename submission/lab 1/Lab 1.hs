...
-- We used the exercise of Constatijn because he did not only test and create the findReversalPrimes, but he also tested the reversal method itself
...
sumSmallestConsecutivePrimes x s | prime (sum consecutivePrimes) = (sum consecutivePrimes) : consecutivePrimes
                                 ...
                                 where consecutivePrimes = (take x (drop s primes)) -- Kan de where ook achter de eerste regel? dan wordt deze clausule niet onnodig uitgevoerd ?en is netter? (weet ik ook niet zeker)
...
-- The function itself does not need to be tested since it will find a answer that statisiefs the given formula.
...
-- Exercise 6
-- We used the exercise of Constatijn because he wrote the most compact code, there were no big differences in our answers
...
EulerNine = do pythagoreanTriples 1000

-- Euler 10
-- We used the exercise of Niels because we prevered the useage of 'primesBelow' over 'takeWhile'
EulerTen = do
